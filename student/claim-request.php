...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $proofFileName = implode(',', $uploadedFiles);

    $insert = $link->prepare("INSERT INTO tblclaimrequests (
        claimID, foundID, studentID, datesubmitted, status, description, lastseenlocation, datelost, estimatedvalue, ownershipproof
    ) VALUES (?, ?, ?, NOW(), 'Pending', ?, ?, ?, ?, ?)");
    ...
    if ($insert->execute()) {
        $countStmt = $link->prepare("SELECT COUNT(*) as total FROM tblclaimrequests WHERE studentID = ?");
        $countStmt->bind_param("i", $studentID);
        $countStmt->execute();
        $countResult = $countStmt->get_result();
        $countData = $countResult->fetch_assoc();
        $totalClaims = $countData['total'];
        $countStmt->close();
        
        $activityMessage = "Submitted " . $totalClaims . " claim request" . ($totalClaims > 1 ? "s" : "");
        $activityMessage .= " (Latest: " . date("M j, Y g:i A") . ")";
        
        $updateActivityStmt = $link->prepare("UPDATE tblactivestudents SET recentactivity = ? WHERE studentID = ?");
        $updateActivityStmt->bind_param("si", $activityMessage, $studentID);
